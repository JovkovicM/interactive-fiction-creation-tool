...
def print_requirements(obj):
    ...
    for c in obj.requirements:
        requirements += c.name + " "
    print(f"You need {requirements}first")
...
def main():
    ...
    program_model = creation_tool_mm.model_from_file(join(this_folder, 'example_world.ct'))
    game_example_model = game_mm.model_from_file(join(this_folder, 'example_playthrough.game'))
